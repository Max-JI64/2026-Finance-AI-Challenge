...
async function loadMarketScenarios() {
  ...
  try {
  const payload = await api(`/api/v3/market-scenarios/${encodeURIComponent(state.selectedArea.code)}/${encodeURIComponent(byId("industry-select").value)}`);
  ...
  try {
    ...
    if (state.scenarioCacheKey !== cacheKey || !state.scenarioResults[state.scenario]) {
      const entries = await Promise.all(Object.keys(scenarioLabels).map(async (scenario) => [
        scenario,
        await api("/api/v3/orchestrate", { method: "POST", body: JSON.stringify({ comparison: { ...body, market_scenario: scenario }, answered_fields: Object.keys(state.eligibilityAnswers), asked_fields: state.v3AskedFields, situation_context: situationContextPayload(), question_round: state.questionBatchRound }) }),
      ]));
...
function renderResults() {
  ...
  renderAlternatives(); renderPolicyDiscovery(); renderCharts(); renderLocalActionBrief(); renderV3ActionPlan();
}
...
function renderV3ActionPlan() {
  const container = byId("v3-action-plan");
  if (!container) return;
  const plan = state.data?.v3?.action_plan;
  if (!plan) {
    container.innerHTML = '<p class="field-help">진단 결과를 바탕으로 실행계획을 준비합니다.</p>';
    return;
  }
  const columns = [
    ["오늘", plan.today],
    ["7일 안에", plan.within_seven_days],
    ["신청 전에", plan.before_application],
  ];
  container.innerHTML = columns.map(([title, items]) => `<article><h3>${escapeHtml(title)}</h3><ol>${(items || []).map((item) => `<li>${escapeHtml(humanizeText(item))}</li>`).join("")}</ol></article>`).join("");
}
...
async function interpretV3Situation() {
  ...
  try {
    const result = await api("/api/v3/situation/interpret", { method: "POST", body: JSON.stringify({ text }) });
