"""Bounded V3 orchestration over the reviewed V2 finance engines."""
...
# The shared V2 cash engine only supports an automatic refinance before/after
# comparison when refinancing happens on the cash-flow reference date. The V2
# UI's reviewed preview assumes execution after four weeks, so V3 must not send
# that incompatible preview into the engine until a remaining-balance baseline
# can be built for the future execution date.
V3_UNSUPPORTED_CONDITIONAL_POLICY_IDS = {"POL_SEMAS_REFINANCE_2026"}
...
def orchestrate_state(request: OrchestrateRequest) -> dict[str, Any]:
    suppressed_conditional_ids = [
        policy_id
        for policy_id in request.comparison.conditional_policy_ids
        if policy_id in V3_UNSUPPORTED_CONDITIONAL_POLICY_IDS
    ]
    comparison = request.comparison.model_copy(
        update={
            ...
            "conditional_policy_ids": [
                policy_id
                for policy_id in request.comparison.conditional_policy_ids
                if policy_id not in V3_UNSUPPORTED_CONDITIONAL_POLICY_IDS
            ],
        }
    )
    # V3 promises no external AI call before the user explicitly requests an
    # action brief. The V2 calculation engine is reused while its policy search
    # tool is fixed to the local SQLite BM25 path inside this V3 process.
    ...
    for candidate in candidates:
        if candidate.get("policy_id") not in suppressed_conditional_ids:
            continue
        readiness = candidate.get("application_readiness") or {}
        readiness["conditional_graph_supported"] = False
        readiness["conditional_graph_status"] = "calculation_unavailable"
        readiness["conditional_graph_reason"] = (
            "대환 실행일의 실제 남은 대출잔액이 필요해 현재 자동 그래프에서는 제외합니다. "
            "실행 전 금융기관에서 대환 가능 원금과 실행일을 확인해 주세요."
        )
    ...
    result["v3"] = {
        ...
        "suppressed_conditional_policy_ids": suppressed_conditional_ids,
        "external_ai_used": False,
