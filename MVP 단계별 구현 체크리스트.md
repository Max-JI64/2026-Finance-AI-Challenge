...
> 문서 상태: Stage 0~6·RE Stage 1~8.1 완료, Gate RE8 재검증 통과·RE Stage 8.2 계획 추가
> 기준일: 2026-08-17
> 실행 체계: `Stage 0~6 + RE Stage 1~9`
> 기준 계획서: `프로젝트 계획서.md`
> 재설계 근거: `프로젝트 차별화 구상.md`
> 다음 실행 단계: RE Stage 8.2 사용자 상황 기반 Hybrid 정책발굴
...
| RE Stage 8 | 완료 | `re8-api-v1.2`·4단계 작업공간·3단계 상권 지도·만원 입력·다중 대안 그래프 |
| RE Stage 8.2 | 계획 | 전체 10개 정책 대상 Hybrid 후보발굴·단계형 자격질문·결정론 시뮬레이션 연결 |
...
| 시작일 | 2026-08-17 |
| 완료일 | 2026-08-17 |
| 상태 | 완료 — `re8-api-v1.2`, 실제사용자 UX 재설계·Gate 재검증 통과 |
...
- [x] 최근 3개월 기본·최대 12개월 월매출 입력과 평균 적용
...
- [x] 사용자 문구는 `보수적으로 보기`·`기준으로 보기`·`회복하는 경우`
...
- [x] 발표용 가게 상황 5종을 한 번 클릭으로 입력·계산·비교
...
현재 RE8.1은 사용자의 상권·업종을 LightGBM 상권 시나리오에, 매출·현금·비용·부채를 현금흐름에 반영한다. 그러나 비교 정책은 `HERO_POLICY_BY_ALTERNATIVE`와 `build_hero`의 고정 대안이므로, 사용자의 전체 상황을 이용해 승인된 10개 정책에서 후보를 동적으로 찾지는 않는다.
...
- [ ] `text-embedding-3-large` 1차 후보와 `text-embedding-3-small` 비용·속도 비교군의 한국어 검색결과 비교
- [ ] 비교결과 공개 후 최종 Embedding 모델·차원·버전 승인
- [ ] OpenAI Embeddings API에 전송할 공개 정책 Chunk와 비식별 상황요약의 정확한 필드 승인
- [ ] 사용자 원금액·사업자 식별정보·자격 응답 원문 비전송 확인
- [ ] 호출비용·Rate limit·실패 Fallback·API 데이터 처리조건 확인
- [ ] 기존 입력에서 자동 파생할 상황정보와 추가로 질문할 자격항목 승인
- [ ] BM25·Vector·Hybrid 비교용 고정 질문·가상 사업장 정답계약 승인
- [ ] Candidate Recall·근거 Hit@k·오추천/누락 점검지표와 통과기준 승인
- [ ] 승인 전 Embedding 생성·사용자 상황 외부전송 금지

### P0 — 현재 입력을 정책발굴용 상황으로 변환

- [ ] 상권·자치구·업종을 정책 Metadata와 공식 업종조건에 매핑
- [ ] 최근 매출 입력에서 매출감소 여부·기간·비율을 파생하되 공식 비교기간과 다르면 `확인 필요` 유지
- [ ] 현금흐름에서 13주 최저현금·고갈시점·안전현금 부족액 파생
- [ ] 임대료·인건비·매입비에서 고정비·변동비 부담 Context 파생
- [ ] 기존 대출에서 잔액·금리·남은 기간·월 상환부담 Context 파생
- [ ] 파생값과 사용자가 직접 확인한 사실을 구분해 가정 원장에 기록
- [ ] 정확한 신용점수·주민등록번호·계좌번호·사업자등록번호를 필수 입력으로 만들지 않음

### P0 — 단계형 추가 질문

- [ ] 초기 화면에서 모든 자격필드를 한꺼번에 요구하지 않음
- [ ] 1차 후보에 필요한 업력·개업일·근로자 수·영업 여부만 우선 질문
- [ ] 임차 고정사업장·세금납부·재해확인서·기존지원·대출실행일 등은 해당 정책 후보가 생겼을 때만 질문
- [ ] 신용조건은 정확한 점수 대신 공고 기준 충족 여부 또는 사용자가 확인할 항목으로 처리
- [ ] 답을 모르면 `모름`을 허용하고 `추가 확인 필요` 상태 유지
- [ ] 질문 이유와 영향을 사용자 문장으로 표시
- [ ] 입력 원값은 세션 밖 DB·검색인덱스·로그에 저장하지 않음

### P0 — OpenAI Embedding Hybrid 검색 인덱스

- [ ] 기존 227개 공식·검토 Chunk와 정책 Metadata·버전·적용기간을 그대로 보존
- [ ] 공개 정책 Chunk를 OpenAI `/v1/embeddings`로 사전 변환하고 모델명·차원·생성일·원문 Hash 기록
- [ ] SQLite에 Vector와 Metadata를 함께 저장하되 원문·Rule·Event를 변경하지 않음
- [ ] 현재 규모에서는 별도 Vector DB 도입 전에 SQLite BLOB + 메모리 내 Cosine 검색을 기준안으로 검증
- [ ] 사용자 질의 시 원금액 대신 `4주 내 현금부족`·`고정비 부담 높음`·`금리 7% 이상` 같은 비식별 범주형 상황요약만 Embedding API에 전송
- [ ] 사용자 질문·상황요약·Embedding 응답을 서비스 DB·검색 인덱스·로그에 영구 저장하지 않음
- [ ] 문서 변경 시 변경 Chunk만 재생성하고 모델 변경 시 전체 Vector 버전 분리
- [ ] PDF 이미지 분석·OCR 없이 기존 텍스트 Chunk만 사용
- [ ] OpenAI Embeddings API 장애·Quota 초과·Timeout 시 BM25 단독으로 즉시 Fallback

### P0 — 후보발굴과 검색 결합

- [ ] BM25는 정책명·기관명·금액·업력·금리·자격용어의 정확 일치를 담당
- [ ] Vector 검색은 원문과 표현이 다른 사용자 상황의 의미 유사성을 담당
- [ ] 자치구·업종·기준일·접수상태·정책버전 Metadata 필터를 검색 전후에 강제
- [ ] BM25와 Vector 순위를 RRF 등 결정론 방식으로 결합
- [ ] Hybrid 점수는 내부 후보발굴에만 사용하고 사용자 적합도 점수로 노출하지 않음
- [ ] 한 정책의 유사 Chunk가 상위 결과를 독점하지 않도록 정책 단위 집계·다양성 제한
- [ ] 전체 10개 정책에서 1차 후보와 후보가 된 이유·일치 근거를 반환

### P0 — LLM과 Workflow Framework의 역할

- [ ] 공식 Rule 자격판정에 Luna 또는 다른 LLM을 사용하지 않음
- [ ] 자유문장 입력 시에만 Luna가 구조화 사실 후보를 추출할 수 있으며 사용자가 확인한 값만 Rule 엔진 입력으로 사용
- [ ] 추가 질문의 쉬운 문장과 정책 설명은 Luna가 만들 수 있지만 질문 필요 여부와 분기조건은 결정론 코드가 결정
- [ ] 기본 구현은 OpenAI SDK + 명시적 Python Service 함수의 2-Step Hybrid RAG로 유지
- [ ] LangChain은 Loader·Retriever 추상화가 실제 중복코드를 줄이는지 검증 전 필수 의존성으로 추가하지 않음
- [ ] LangGraph는 장시간 실행·중단 후 재개·다중 Tool 자율선택이 필요한 후속 Agent 범위 전에는 도입하지 않음
- [ ] Framework 이름이 아니라 LightGBM·OpenAI Embedding·Luna의 실질 기능과 평가결과를 AI 적용 근거로 제시

### P0 — 공식 Rule·금융 Event·순위 연결

- [ ] Hybrid 검색은 후보를 찾을 뿐 적격·부적격을 결정하지 않음
- [ ] 후보마다 RE6 공식 Rule 엔진을 전체 적용
- [ ] 명확한 부적격은 제외하고 미입력 조건은 `추가 확인 필요`로 유지
- [ ] 접수 종료와 자격 부적격을 별도 상태로 유지
- [ ] 검수된 금융 Event가 있는 정책만 13주·6개월 그래프와 부채표에 연결
- [ ] 금액·지급일·금리·상환조건이 미확인인 정책은 임의 값으로 시뮬레이션하지 않음
- [ ] 최종 순위는 RE7의 최소부채·최장생존·최소상환·빠른실행 결정론 규칙만 사용
- [ ] Embedding·BM25·Luna가 자격판정·현금계산·최종순위를 변경하지 못하도록 Guard

### P0 — AI 질의 근거검색

- [ ] 선택 정책 내부에서도 BM25와 Vector 근거검색을 함께 사용
- [ ] 정책 ID·버전·기준일 필터 후 공식 근거만 Luna에 전달
- [ ] 동일 URL은 페이지·절을 묶어 중복 링크를 제거
- [ ] Luna는 후보선정 이유·확인사항·Trade-off·신청절차 설명만 담당
- [ ] Fact-lock 불일치 응답 폐기와 로컬 Fallback 유지
- [ ] Luna 실패 시 후보·자격·계산·공식 링크 유지

### P0 — 비교평가와 오류분석

- [ ] 정확 정책명·기관명·금액 질문
- [ ] 같은 의미의 일상어·동의어·우회표현 질문
- [ ] 오타·띄어쓰기·줄임말 질문
- [ ] 상권·업종·매출감소·현금부족·고금리대출이 결합된 가상 사업장
- [ ] 명확한 부적격·접수 종료·정보 부족 사례
- [ ] BM25 단독·Vector 단독·Hybrid를 동일 정답계약으로 비교
- [ ] 정책 Candidate Recall@k·근거 Hit@k·MRR·중복률·응답시간 기록
- [ ] 적격정책 누락·부적격정책 승격·다른 버전 근거 혼입을 별도 오류로 기록
- [ ] 현재 8문항 검색평가는 참고값으로만 보존하고 확대 평가를 통과기준으로 사용
- [ ] 결과를 모두 공개한 뒤 Hybrid 채택·조정·보류를 사용자 승인

### 완료 산출물

- [ ] 승인된 RE8.2 계약서와 개인정보 전송범위
- [ ] OpenAI Embedding 모델·차원·API 조건·원문 Hash Manifest
- [ ] Hybrid SQLite 스키마와 재생성 스크립트
- [ ] 사용자 상황 → 후보발굴 API
- [ ] 단계형 추가질문 UI
- [ ] 공식 Rule·금융 Event·RE7 순위 연결 테스트
- [ ] BM25·Vector·Hybrid 비교평가 보고서
- [ ] 오류사례와 Fallback 보고서
- [ ] RE8.2 버전 Manifest

### Gate RE8.2

- [ ] 사용자의 기존 입력이 전체 10개 정책 후보발굴에 실제 반영됨
- [ ] 승인된 추가 질문만 단계적으로 표시됨
- [ ] Hybrid가 승인된 검색평가 기준을 충족하거나 미달 상태가 명시됨
- [ ] 명확한 부적격 정책이 실행 가능한 추천으로 승격된 사례 0건
- [ ] 정책버전·적용기간이 다른 근거 혼입 0건
- [ ] 미확인 금융조건을 임의로 계산한 사례 0건
- [ ] Embedding·Luna가 자격·계산·순위를 변경한 사례 0건
- [ ] OpenAI Embeddings API 장애 시 BM25 Fallback으로 후보·공식 링크 유지
- [ ] 사용자 원금액·자격 원값·질문이 영구저장된 사례 0건
- [ ] RE9 기능명세·배포 QA·가상 페르소나 검증항목에 Hybrid 흐름 반영
