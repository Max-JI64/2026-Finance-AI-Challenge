...
> 문서 상태: Stage 0~6·RE Stage 1~8.3 완료·종료, V2 사용자 직접 검토·수정 진행, 외부 배포 중단
> 기준일: 2026-08-17
> 실행 체계: `Stage 0~6 + RE Stage 1~8.2 + RE Stage 8.3 + RE Stage 9`
> 기준 계획서: `프로젝트 계획서.md`
> 재설계 근거: `프로젝트 차별화 구상.md`
> 다음 실행 단계: 사용자가 로컬 V2 화면의 수정 지점을 지정하면 V2 변경 원장에 `V2-006`부터 기록해 항목별 구현·내부 회귀검증·사용자 화면 재확인
...
- [x] 6개월 월별 현금흐름
- [x] 무대응 시나리오
- [x] 하방·기준·회복 매출환경 시나리오 또는 검증된 사용자 충격률 Fallback
- [x] 공식 정책 10개 구조화
- [x] 적격·부적격·추가 확인 필요 자격판정
- [x] 보조금·바우처·이차보전·융자·대환·보증의 지원유형 구분
- [x] 정책 적용 전후 현금잔액·부채·이자 비교
- [x] 최소부채·최장생존·최소상환·빠른실행 목표 중 선택
- [x] 공식 근거 RAG
- [x] 생성형 AI의 근거·Trade-off 설명
- [ ] 실제 접근 가능한 웹 URL
- [x] 기능명세서와 샘플 검증 절차
- [x] 13주 생존 시뮬레이션 Hero 화면
...
| RE Stage 7 | 완료 | `re7-v1` 동일 기준 비교·목표정렬·Pareto·실행계획·대표 상세 시나리오 |
| RE Stage 8 | 완료 | `re8-api-v1.3`·4단계 작업공간·3단계 상권 지도·만원 입력·다중 대안 그래프 |
| RE Stage 8.2 | 완료·Gate 통과 | 17개 정책 Hybrid 후보발굴, 단계형 질문, Rule/Event 경계, 동적 대안, 40건 평가와 개인정보·fallback 계약 완료 |
| RE Stage 8.3 | 완료·종료 | 사용자 최종 승인으로 닫았으며 V2 전환 후 재개하지 않음 |
| RE Stage 9 | 보류 | V2 검토 종료와 별도 배포 승인 전 실행 금지; 기존 로컬 QA는 수정 전 기준선으로 보존 |
...
- [x] 월 금액만 받는 간편모드는 특정 고갈 주차 확정 대신 추정 범위 표시
- [x] 간편 추정 범위 산출법 사용자 승인 — 2026-08-16
- [x] 매출 입금유형 `매일·매주·월초·월중·월말` 입력
- [x] 주요 비용·대출상환 시기 `초·중·말` 입력
- [x] `초=1~10일`, `중=11~20일`, `말=21일~월말` 변환
- [x] 보수적 일정은 유입 늦게·유출 일찍 배치
- [x] 기준 일정은 허용구간 중앙일 배치
- [x] 완화 일정은 유입 일찍·유출 늦게 배치
- [x] 세 일정을 날짜별 이벤트로 변환해 기존 `re3-v1`을 반복 호출
- [x] 정확한 날짜 입력은 범주형 가정보다 우선
- [x] 기준 현금곡선과 보수적~완화 범위를 함께 표시
- [x] 현재 `re3-v1`처럼 주요 입금일·지급일을 받은 경우 `반복일정 가정 기반 계산` 표시
- [x] 날짜별 이벤트·대출 CSV는 `상세 일정 기반 계산` 표시
- [x] 입력 수준·일정 가정·불확실성·계산 버전 공통 표시
- [x] 임의의 `9~11주` 범위 생성 금지
...
| 시작일 | 2026-08-16 |
| 완료일 | 2026-08-16 |
| 상태 | `re7-v1` 구현 완료·Gate RE7 통과; 실제 화면 토글·배치는 RE8 전달 |
...
- [x] RE7 시나리오 생성기와 정렬 엔진에 후보상태 적용
...
- [x] 2026-08-16 사용자 재개 요청에 따라 승인 계약 그대로 `re7-v1` 구현
...
- [x] 지급일정이 포함된 검증 가상 사업장 사용
- [x] `03_declining_cash_shortage.json`을 출발점으로 상세 대출·지급일정 보완
- [x] 정책 적용 전 서로 다른 현실적 상세 샘플 2~3개 입력 동결
- [x] `생존`을 현금잔액 0원 이상 유지기간으로 정의하고 실제 존속·폐업 예측과 구분
- [x] 무대응·보조금·비용절감·대환·정책자금·복합안을 같은 현금흐름 축에서 비교
- [x] 비차입 후보로 위기 소상공인 Track2 사후정산 보조금 검토
- [x] 부채구조 개선 후보로 소상공인 정책자금 대환대출 검토
- [x] 신규자금 후보로 서울시 긴급자영업자금 검토
- [x] 복합안은 비차입 지원 + 비용 5% 절감 + 안전현금 최소대출
- [x] 자격·동시수혜·중복지원·지급일·승인금액 가정 검증 후 미확인 동시수혜는 조건부로 확정
- [x] 동시 현금곡선은 무대응·비차입·신규자금·복합안 최대 4개
- [x] 대환·비용절감 실제 화면 토글 제공 — RE8
- [x] 13주 현금곡선을 화면 중심에 표시 — RE8
- [x] 6개월 유지 여부·신규부채·월상환·총이자를 함께 반환
- [x] 월 금액만 입력한 간편모드 결과를 정확한 주차 데모로 사용 금지
- [x] 원하는 홍보 숫자에 맞춘 샘플 입력 역산 금지
...
- [x] 최소상환
- [x] 빠른실행
- [x] 승인된 4주 필수지출 기반 안전현금 제안값과 직접 입력 UI 구현 — 계산·수정 입력계약 완료, 실제 UI는 RE8
- [x] 목표별 결과 의미 설명
...
- [x] 승인되지 않음 시나리오
- [x] 지급 지연 시나리오
- [x] 부적격·접수 종료 정책 비교·정렬 제외
- [x] 추가 확인·현재성 미확인 정책의 명시적 조건부 시뮬레이션
- [x] 조건부 정책의 기본 1순위·빠른실행 1순위 차단
...
사용자가 웹에서 사업장 선택부터 재무 입력, 현금 진단, 정책금융 영향 비교와 실행계획까지 네 단계로 완료하고 핵심 현금·부채 차이를 바로 이해할 수 있게 한다.
...
| 시작일 | 2026-08-17 |
| 완료일 | 2026-08-17 |
| 상태 | 완료 — `re8-api-v1.3`, 실제사용자 UX 재설계·Gate 재검증 통과 |
...
- [x] Health
- [x] 서비스 계약
- [x] 동결된 RE1의 옛 명칭을 보존하되 현재 API·화면 서비스명은 `정책금융 영향 시뮬레이터`로 이관
- [x] 상권 목록
- [x] 업종 목록
- [x] 기준 현금흐름
- [x] 외부환경 시나리오
- [x] 상권 중심좌표·면적·자치구·행정동 지도 목록
- [x] 정책 자격
- [x] 정책금융 영향 시뮬레이션
- [x] 대안 비교
- [x] AI 질의응답
...
- [x] 공식 근거
- [x] 한계
- [x] 자격판정과 접수상태를 `eligibility_status`·`availability_status`로 분리
- [x] `candidate_state`·`reason_summary`·`items_to_confirm`
- [x] `as_of_date`·`policy_version`·공식 공고/신청 URL
- [x] Rule ID·Chunk ID·BM25 점수·검색순위는 사용자 응답에서 제외

### P0 — 화면 1 사업장 선택

- [x] 서울시 상권분석서비스 원본의 현행 상권 1,596개
- [x] 서울 전체 25개 자치구 원 → 행정동 원 → 개별 상권 원의 단계형 지도
- [x] 자치구·행정동 집계 원 Hover·Popup에 지역명과 포함 상권 수 표시
- [x] OSM 배경과 대비되는 코발트색 원·흰 외곽 링, 자치구·행정동·개별 상권 상시 라벨 적용
- [x] 자치구 선택보다 앞선 서울 전체 상권명 검색과 선택 지역 자동 이동
- [x] 지도 중심과 확대 수준만으로 자치구→행정동→개별 상권 자동 선택
- [x] 폴리곤 대신 공식 중심좌표와 면적 비례 원 표시
- [x] 원 Hover·Popup에 상권명·자치구·행정동·상권유형·면적 표시
- [x] 자치구 선택 시 상권 목록 필터와 지도 범위 이동
- [x] 상권 목록 선택 시 지도 이동·원 강조·Popup 열기
- [x] 지도 원 클릭 시 자치구·상권 목록 선택 동기화
- [x] 업종·영업기간 선택
- [x] 지도 실패 시 목록 선택 유지

### P0 — 화면 2 재무 입력

- [x] 최근 6개월(2개 분기) 기본·최대 12개월 월매출 입력과 평균 적용
- [x] 현재 현금·임대료·인건비·필수매입비
- [x] 기존 대출 잔액·금리·잔여기간
- [x] `1~10일`, `11~20일`, `21일~월말` 시기 선택
- [x] 원 단위 안내·필수값 검증
- [x] 한글 거래내역·대출 CSV 양식과 영문 기존 양식 호환
- [x] 화면 금액 입력과 한글 CSV 금액을 만원 단위로 통일
- [x] 한글 CSV 다운로드를 UTF-8-SIG로 제공하고 기존 원 단위 헤더도 호환
- [x] 여러 달·분기의 실제 입출금 날짜 입력 허용

### P0 — 화면 3 현금 진단

- [x] RE5 LightGBM을 실제 선택 상권×업종에 연결
- [x] 사용자 문구는 `하방 범위`·`기준 범위`·`회복 범위`로 표시하고 각 범위의 의미 설명
- [x] 모델 입력 기준분기가 실시간 자료나 개별 점포 실적이 아니라는 설명
- [x] 내부 P10·P50·P90·Coverage·Holdout 지표 비노출
- [x] 집계 변화율이 내 점포 매출 예측이 아니라는 설명
- [x] 현재 현금·28일 안전현금·첫 안전현금 하회 주차·13주 말 현금
- [x] Y축 `현금 잔액(만원)`·X축 주차·안전현금 영역·0원 기준
- [x] 6개월 말 현금·남은 대출·만기까지 총이자
- [x] 선택 시나리오가 반영된 무대응 수치와 대안 화면 수치 일치
- [x] 하방·기준·회복 13주 현금곡선을 동시에 표시하고 선택 범위만 강조
- [x] 상권 범위 변경 시 결과 KPI·그래프로 자동 스크롤하고 달라지는 KPI만 색상 강조
- [x] 28일 필수지출 기반 안전현금 금액은 상권 범위와 무관하게 고정됨을 표시
- [x] 조건부 정책 비교 가정 선택

### P0 — 화면 4 대안과 실행

- [x] 목표를 `빚을 덜 늘리기`·`현금을 오래 유지하기`·`매달 갚는 돈 줄이기`·`효과가 빨리 나타나는 안`으로 설명
- [x] 무대응 포함 대안 카드에 13주·6개월 현금, 신규부채, 월 최대상환 표시
- [x] 무대응 대비 13주 현금 차이 표시
- [x] 선택 대안과 무대응 두 곡선 우선 비교
- [x] 모든 수치는 접힌 상세표로 제공
- [x] 대안 클릭 시 상세 Popup과 확인조건 표시
- [x] Popup에서 해당 실행계획과 정책 질문으로 바로 이동
- [x] Popup 닫기 버튼과 바깥 배경 클릭 닫기
- [x] 카드·그래프 선택은 Popup을 열지 않고 초록색 `자세히 보기` 버튼만 Popup 실행
- [x] 모든 대안 곡선을 한 그래프에 표시하고 선택 대안만 초록색, 나머지는 회색 처리
- [x] 자격 확인 전 정책의 추천 순위 제외와 1순위 대안의 기간 중 음수 현금 경고 표시
- [x] 실행계획을 별도 섹션에서 제거하고 상세 Popup에 확인조건·서류·문의·공식 링크 통합
- [x] 공식 링크를 사이트명과 순번으로 구분
- [x] 발표용 가게 상황 5종을 접힌 영역에서 한 번 클릭해 6개월 값을 입력하되 현재 화면 유지
- [x] 우선 확인조건·신청기한·서류·지급 전 필요현금·공식 링크
- [x] 공식근거 AI 질문과 로컬 Fallback
- [x] 발표용 정책 질문 5종을 접힌 영역에서 버튼으로 질문창에 입력
...
- [x] 단위 오류
- [x] 현금흐름 계산 불가
- [x] 외부모델 실패 시 변화율 0% 결정론 Fallback과 상태 표시
- [x] RAG 실패 시 계산결과·공식링크 유지
- [x] LLM 실패 시 구조화 결과 유지
- [x] Fact-lock 검증 실패 LLM 응답 폐기

### P0 — 보안·로그

- [x] 환경변수 비밀키
- [x] 민감 입력 로그 제외
- [x] 세션 자격 프로필 원값·정확한 신용점수 로그/DB/검색 인덱스 저장 금지
- [x] 요청 크기 제한
- [x] CSV 파일 형식 제한
...
- [x] 민감정보 로그 미기록
- [x] 외부서비스 장애 Fallback
- [x] 상권 지도·자치구·상권 목록 양방향 연동
- [x] 13주 진단과 대안 무대응 수치 일치
- [x] 390×844 문서 전체 가로 넘침 없음
...
- [x] 입력부터 실행계획까지 한 흐름으로 완료
- [x] 계산·정책·모델·RAG 버전 추적
- [x] 핵심 오류·Fallback 동작
- [x] 점수·승인확률로 오해할 표현 없음
- [x] 민감정보 비저장 확인
- [x] 후속 단계 영향 검토와 필요 수정 완료
...
RE8.1의 비교 정책은 `HERO_POLICY_BY_ALTERNATIVE`와 `build_hero`의 고정 대안이었다. RE8.2는 검토된 17개 정책을 동적으로 검색하고, 후보별 공식 Rule 상태와 검수 가능한 금융 Event만 현금 그래프에 연결한다.

이 단계는 검색창만 개선하는 작업이 아니다. 사용자 입력을 정책 후보발굴에 연결하고, 정확한 정책용어 검색과 상황의 의미 검색을 결합한 뒤, 공식 Rule 판정과 금융 Event 시뮬레이션을 거쳐 대안을 생성하도록 서비스 흐름을 확장한다.

### 목표

사용자의 구조화 입력과 상황 의미를 함께 사용해 전체 정책 포트폴리오에서 누락을 줄인 후보를 찾되, 자격판정·현금계산·최종순위는 검수된 결정론 엔진만 담당하게 한다.

### 상태 기록

| 항목 | 기록 |
| --- | --- |
| 담당자 | 사용자 계약·모델·평가기준 승인 / Codex 구현·QA 지원 |
| 시작일 | 2026-08-17 |
| 완료일 | 2026-08-17 |
| 상태 | **완료·Gate 통과** — 승인된 `text-embedding-3-large` Hybrid, 단계형 자격질문, 17개 Rule 상태, 검수 Event 동적 대안, 40건 평가를 완료 |
| 선행조건 | Gate RE8 |
| 후속조건 | Gate RE8.2 통과 전 RE9 배포 착수 금지 |

### 2026-08-17 실제 구현 현황 감사

아래 표는 코드·SQLite DB·보고서·테스트를 다시 대조한 완료 상태다. `검색 후보`와 `실행 가능한 추천 대안`은 계속 분리하며, 자격·접수·Event 조건을 통과한 후보만 대안으로 연결한다.

| 영역 | 상태 | 현재 실제 동작 | 남은 작업 |
| --- | --- | --- | --- |
| 정책 원문·DB | 완료 | 사용자 검토 Markdown 17개 정책을 817개 Chunk로 저장한다. HTML 본문은 색인하지 않고 공식 URL만 저장한다. 2026-08-17 추가분은 노란우산·입원 생활비·민간 아이돌봄·중구 맞춤형 지원 4개다. | 산재보험료 지원은 잘못 복사된 노란우산 문서를 삭제했으므로 올바른 Markdown 적재 후 추가 |
| Embedding 저장 | 완료 | `text-embedding-3-small` 1,536차원 817행과 승인된 `text-embedding-3-large` 3,072차원 817행을 모델별로 보존한다. | 없음 |
| Hybrid 검색 엔진 | 완료 | `text-embedding-3-large` Vector와 BM25를 RRF로 결합하고 요청당 5초·최대 2회 후 BM25로 전환한다. | RE9 배포환경 API 키·Usage tier 확인 |
| 검색 비교평가 | 완료 | 검색 32건·안전성 8건에서 Hybrid-large Recall@5 1.000·Hit@5 1.000·MRR 0.872, 신규 정책 Hit@5 1.000, 안전성 8/8이다. | RE9 회귀 QA에서 고정 결과 재확인 |
| 사용자 상황 변환 | 완료 | 자치구명·업종 대분류·매출·고정비·부채·현금위험 구간·목표만 비식별 문장으로 전송한다. 상권명과 원금액은 제외한다. | 없음 |
| 후보발굴 API·화면 | 완료 | 17개 Metadata의 자치구·접수·적용기간·버전을 필터링하고 상위 6개 후보, 근거, 공식 링크, 자격·접수 상태를 표시한다. | 없음 |
| 자격판정 | 완료 | 기존 10개 RE6 Rule과 추가 7개 RE8.2 overlay를 결정론적으로 적용해 부적격·추가 확인·접수종료를 분리한다. | 공식 공고 변경 시 사용자 검수 후 버전 갱신 |
| 검색 전용 7개 정책 | 완료 | 7개 모두 Rule과 Event 상태를 구조화했다. 사업체 현금과 분리해야 하는 개인 생활비·공제 적립·돌봄서비스, 6개월 밖 효과, 접수종료는 대안 승격을 차단한다. | 산재보험은 올바른 Markdown 적재 전 범위 밖 |
| 현금 그래프·순위 연결 | 완료 | 고용보험 등급 또는 가족친화 기업지원 실제 금액·지급일이 입력된 검수 Event만 동적 대안으로 만들고 RE7 결정론 순위를 적용한다. | 없음 |
| AI 정책 채팅 | 완료 | 전체 17개 또는 선택 정책 Hybrid 근거검색, 페이지 메모리 5회 제한, 로컬 민감패턴 제거, 공식 URL 근거와 Luna fact-lock을 유지한다. | 없음 |
| AI 정책 갱신 보조 | 후속 강화 후보 | 자동 Rule/Event 추출은 이번 승인 계약과 Gate 밖이며 자동 판정에 사용하지 않는다. | RE9 이후 별도 사용자 승인으로 검토 |

### 현재 서비스 흐름과 아직 연결되지 않은 흐름

현재 동작:

`사용자 입력 → 승인된 비식별 범주형 상황요약 → OpenAI Embedding + BM25 RRF Hybrid → Metadata 필터 → 상위 6개 후보·근거`

`검색 후보 → 필요한 자격질문 → 결정론 Rule 판정 → 검수 Event와 사용자 금융조건 → 현금·부채 시뮬레이션 → RE7 순위`

`Embedding 최초 호출·오류 재시도까지 실패 → BM25 후보·공식 링크 유지`

### 완료한 순서

1. 최종 모델·전송 필드·질문 범위·40건 평가와 통과기준을 사용자 승인했다.
2. Hybrid, 5초 Timeout, 사용자 동작당 최대 2회, BM25 Fallback을 활성화했다.
3. 17개 정책 Metadata와 기준일·자치구·버전 필터를 구조화했다.
4. 후보별 단계형 질문과 `예/아니오/모름` 상태를 구현했다.
5. 기존 10개 RE6 Rule과 추가 7개 overlay Rule/Event 경계를 연결했다.
6. 검수 Event와 실제 사용자 금융조건이 있는 후보만 동적 대안으로 연결했다.
7. 검색 32건·안전성 8건을 평가하고 개인정보 비저장·fallback을 자동 검증했다.
8. 승인 계약서·최종 Manifest·RE9 영향사항을 동결하고 Gate RE8.2를 통과 처리했다.

### 현재 구현 증거

- 로컬 DB: `rag/index/policy_re8.sqlite3` — Manifest와 SHA-256 일치
- DB 재생성: `scripts/build_re_stage8_2_hybrid_db.py`
- 검색 엔진: `src/rag/hybrid_search.py`
- OpenAI Embedding Client: `src/rag/openai_embeddings.py`
- 사용자 상황·후보 API 연결: `src/integration/re_stage8.py`의 `_situation_summary`·`_discover_policies`
- 후보 화면: `app/static/app.js`의 `renderPolicyDiscovery`
- 평가 정답계약: `data/samples/re_stage8_2/hybrid_evaluation_cases.json`
- 평가 결과: `reports/re_stage8_2/hybrid_retrieval_evaluation.json`·`hybrid_retrieval_evaluation.md`
- 승인 계약: `reports/re_stage8_2/approved_contract.md`
- 최종 Manifest: `reports/re_stage8_2/manifest.json` — `gate_passed=true`
- 자동 테스트: `tests/test_re_stage8_2.py`의 재시도·Rule·Event·비식별·fallback 검증과 전체 회귀 테스트, `app/static/app.js` 문법 검사
- 런타임 상태: `text-embedding-3-large` 3,072차원 Hybrid, 사용자 동작당 최대 2회 후 BM25 Fallback
- Rule 연결 현황: 색인 정책 17개 중 기존 10개 RE6 + 추가 7개 RE8.2 overlay

### 착수 전 필수 사용자 결정

- [x] Embedding 제공자는 OpenAI Embeddings API로 결정 — 2026-08-17 사용자 지시
- [x] `text-embedding-3-large` 1차 후보와 `text-embedding-3-small` 비용·속도 비교군의 한국어 검색결과 비교 — 24문항 동일 정답계약
- [x] 비교결과 공개 후 `text-embedding-3-large` 3,072차원 Hybrid 승인 — 2026-08-17
- [x] OpenAI Embeddings API에 전송할 공개 정책 Chunk와 비식별 상황요약의 정확한 필드 승인
- [x] 사용자 원금액·사업자 식별정보·자격 응답 원문 비전송 구현·테스트 확인
- [x] 호출비용·Usage tier별 Rate limit·5초 Timeout·최대 2회·BM25 Fallback·API 데이터 처리조건 확인
- [x] 기존 입력에서 자동 파생할 상황정보와 추가로 질문할 자격항목 승인
- [x] BM25·Vector·Hybrid 비교용 40건 고정 정답계약 승인
- [x] Recall@5 0.95·Hit@5 0.95·MRR 0.80과 안전성 통과기준 승인
- [x] 공개 정책 Markdown Embedding 생성과 승인 모델 런타임 활성화 완료

### P0 — 현재 입력을 정책발굴용 상황으로 변환

- [x] 자치구·업종을 정책 Metadata와 공식 업종조건에 매핑하되 상권명은 외부 전송에서 제외
- [x] 최근 월 매출 입력에서 매출감소·보합·증가 Context 파생
- [x] 현금흐름에서 13주 최저현금·고갈시점·안전현금 부족 Context 파생
- [x] 임대료·인건비에서 고정비 부담 Context 파생
- [x] 기존 대출에서 잔액 존재·금리 기준의 대환 필요 Context 파생
- [x] 파생 상황값과 사용자가 직접 확인한 Event 금융조건을 구분해 대안 가정 원장에 기록
- [x] 정확한 신용점수·주민등록번호·계좌번호·사업자등록번호를 필수 입력으로 만들지 않음

### P0 — 단계형 추가 질문

- [x] 초기 화면에서 모든 자격필드를 한꺼번에 요구하지 않음
- [x] 1차 후보에 필요한 기업 규모·개업일·근로자 수·영업 여부만 우선 질문
- [x] 임차 고정사업장·세금납부·재해확인서·기존지원 등은 해당 정책 후보가 생겼을 때만 질문
- [x] 신용조건은 정확한 점수 대신 공고 기준 충족 여부 또는 사용자가 확인할 항목으로 처리
- [x] 답을 모르면 `모름`을 허용하고 `추가 확인 필요` 상태 유지
- [x] 질문 이유와 영향을 사용자 문장으로 표시
- [x] 입력 원값은 세션 밖 DB·검색인덱스·로그에 저장하지 않음

### P0 — OpenAI Embedding Hybrid 검색 인덱스

- [x] 검토된 17개 정책 Markdown을 817개 Chunk로 재구성하고 Metadata·버전·공식 링크 보존
- [x] 공개 정책 Chunk를 OpenAI `/v1/embeddings`로 사전 변환하고 모델명·차원·원문 Hash 기록
- [x] SQLite에 Vector와 Metadata를 함께 저장하되 원문·Rule·Event를 변경하지 않음
- [x] 현재 규모에서 SQLite BLOB + 메모리 내 Cosine 검색 검증
- [x] 사용자 원금액 대신 `13주 내 현금 부족`·`고정비 부담 높음`·`고금리 기존 대출` 같은 비식별 범주형 상황요약만 전송하도록 구현
- [x] 사용자 질문·상황요약·Embedding 응답을 서비스 DB·검색 인덱스·로그에 영구 저장하지 않음
- [x] 문서 Hash가 같은 Chunk는 기존 Vector를 재사용하고 모델별 Vector 버전 분리
- [x] PDF 이미지 분석·OCR 없이 사용자 검토 Markdown만 검색·Embedding
- [x] OpenAI Embeddings API 장애·Quota 초과·Timeout 시 최초 1회 + 오류 재시도 1회, 총 최대 2회 후 BM25 Fallback 구현·테스트

### P0 — 후보발굴과 검색 결합

- [x] BM25는 정책명·기관명·금액·업력·금리·자격용어의 정확 일치를 담당
- [x] Vector 검색은 원문과 표현이 다른 사용자 상황의 의미 유사성을 담당
- [x] 검색 함수에 정책 ID·정책버전·기준일 필터 인자와 적용기간 필터 로직 구현
- [x] 17개 정책의 자치구·공식 업종조건·접수상태·실제 적용기간 Metadata를 채우고 검색 전후에 강제
- [x] BM25와 Vector 순위를 RRF 결정론 방식으로 결합
- [x] Hybrid 점수는 내부 후보발굴에만 사용하고 사용자 적합도 점수로 노출하지 않음
- [x] 한 정책의 유사 Chunk가 상위 결과를 독점하지 않도록 정책 단위 다양성 제한
- [x] 전체 17개 정책에서 1차 후보와 후보가 된 이유·일치 근거를 반환

### P0 — LLM과 Workflow Framework의 역할

- [x] 공식 Rule 자격판정에 Luna 또는 다른 LLM을 사용하지 않음
- [x] Luna는 자유질문 설명에만 사용하고 구조화 자격값을 자동으로 Rule 입력에 넣지 않음
- [x] 추가 질문 필요 여부·문장·분기조건을 검수된 결정론 코드가 결정
- [x] 기본 구현은 최소 OpenAI API Client + 명시적 Python Service 함수의 2-Step Hybrid RAG로 유지
- [x] LangChain은 Loader·Retriever 추상화가 실제 중복코드를 줄이는지 검증 전 필수 의존성으로 추가하지 않음
- [x] LangGraph는 장시간 실행·중단 후 재개·다중 Tool 자율선택이 필요한 후속 Agent 범위 전에는 도입하지 않음
- [x] Framework 이름이 아니라 LightGBM·OpenAI Embedding·Luna의 실질 기능과 평가결과를 AI 적용 근거로 제시

### 후속 강화 후보 — AI 정책 갱신 보조

- [ ] 새 정책 Markdown에서 자격조건·지원금액·지급시점·상환조건·접수기간을 구조화 후보로 추출
- [ ] 기존 검토 Rule·Event·정책버전과 비교해 변경·충돌·필수값 누락을 표시
- [ ] LLM 추출값은 자동 판정·계산에 사용하지 않고 사용자 검수·승인 후에만 반영
- [ ] 정책별 검수 정답표를 만들고 필드 정확도·누락률·근거절 일치율의 승인 기준을 확정
- [ ] 단순 채팅 추가가 아니라 신규 정책 온보딩 시간과 구조화 누락을 줄이는 효과를 평가

### P0 — 공식 Rule·금융 Event·순위 연결

- [x] Hybrid 검색은 후보를 찾을 뿐 적격·부적격을 결정하지 않음
- [x] 기존 10개 후보는 RE6 공식 Rule, 추가 7개는 분리된 RE8.2 검수 overlay 적용
- [x] 명확한 부적격은 제외하고 미입력 조건은 `추가 확인 필요`로 유지
- [x] 접수 종료와 자격 부적격을 별도 상태로 유지
- [x] 기존 RE7 고정 대안에서 검수된 금융 Event가 있는 기존 정책만 13주·6개월 그래프와 부채표에 연결
- [x] Hybrid 후보 중 검수 Event와 사용자 금융조건이 있는 고용보험·가족친화 기업지원만 그래프·부채표에 동적으로 연결
- [x] 금액·지급일·금리·상환조건이 미확인인 신규 정책은 임의 값으로 시뮬레이션하지 않음
- [x] 최종 순위는 RE7의 최소부채·최장생존·최소상환·빠른실행 결정론 규칙만 사용
- [x] Embedding·BM25·Luna가 자격판정·현금계산·최종순위를 변경하지 못하도록 Guard

### P0 — AI 질의 근거검색

- [x] 선택 정책 내부에서도 승인된 `text-embedding-3-large` Hybrid 근거검색 사용, 실패 시 BM25 실행
- [x] 정책을 지정하지 않으면 추천 목록 밖을 포함한 전체 17개 정책에서 관련 정책을 탐색
- [x] 단답형 결과를 현재 페이지 메모리 기반 채팅으로 변경하고 사용자 질문을 5회로 제한
- [x] 질병·입원과 육아·돌봄 상황형 발표 질문 예시 2개 제공
- [x] 질병·돌봄 등 일상어를 정책 용어로 보강하는 결정론적 한국어 검색 확장 적용
- [x] 서비스 화면의 채팅 기록을 서버 DB·검색 인덱스·`localStorage`·`sessionStorage`에 저장하지 않음
- [x] 정책 ID·버전·기준일 필터 후 공식 근거만 Luna에 전달
- [x] 동일 URL은 페이지·절을 하나의 공식 공고 링크로 묶어 중복 링크 제거
- [x] Luna는 후보선정 이유·확인사항·Trade-off·신청절차 설명만 담당
- [x] Fact-lock 불일치 응답 폐기와 로컬 Fallback 유지
- [x] Luna 실패 시 후보·자격·계산·공식 링크 유지

### P0 — 비교평가와 오류분석

- [x] 정확 정책명·기관명·금액 질문
- [x] 같은 의미의 일상어·동의어·우회표현 질문
- [x] 오타·띄어쓰기·줄임말 질문
- [x] 상권·업종·매출감소·현금부족·고금리대출이 결합된 가상 사업장
- [x] 명확한 부적격·접수 종료·정보 부족 사례
- [x] BM25 단독·Vector 단독·Hybrid를 동일 검색 32문항 정답계약으로 비교
- [x] 정책 Candidate Recall@5·Hit@5·MRR·응답시간 기록
- [x] 적격정책 누락·부적격정책 승격·다른 버전 근거 혼입을 별도 안전성 결과로 기록
- [x] 기존 24문항에 신규 4개 정책 검색 8건과 안전성 8건을 더한 40건 계약 작성
- [x] 40건 정답계약과 통과기준을 사용자 승인하고 부적격·접수종료·정보부족·버전혼입·fallback 평가
- [x] 결과 공개 후 Hybrid-large 채택 승인

### 완료 산출물

- [x] 승인된 RE8.2 계약서와 개인정보 전송범위
- [x] 구현 상태 Manifest — 17개 Markdown 원문 Hash, DB Hash, small/large 모델·차원·행 수, 기존 24문항 임시 평가, Gate Blocker 기록
- [x] 최종 승인 Manifest — 선택 모델·차원·호출비용·Rate limit·Timeout·API 데이터 처리조건과 승인된 전송 필드 동결
- [x] Hybrid SQLite 스키마와 증분 재생성 스크립트
- [x] 사용자 상황 → 후보발굴 API·화면
- [x] 단계형 추가질문 UI
- [x] 공식 Rule·금융 Event·RE7 순위 연결 테스트
- [x] BM25·Vector·Hybrid 비교평가 보고서
- [x] Embedding 실패 BM25 Fallback 자동 테스트
- [x] `reports/re_stage8_2/manifest.json` 최종 Manifest 생성 — 상태 `completed`, `gate_passed=true`

### Gate RE8.2

- [x] 승인된 사용자 입력 필드가 전체 17개 정책 후보발굴에 실제 반영됨
- [x] 승인된 추가 질문만 단계적으로 표시됨
- [x] Hybrid-large가 Recall@5 1.000·Hit@5 1.000·MRR 0.872로 승인 기준 충족
- [x] 명확한 부적격·접수종료 정책이 실행 가능한 추천으로 승격된 사례 0건
- [x] 정책버전·적용기간이 다른 근거 혼입 0건
- [x] 미확인 금융조건을 임의로 계산한 사례 0건 — 조건 미입력과 사업체 현금 비대상 Event 차단
- [x] Embedding·Luna가 자격·계산·순위를 변경한 사례 0건
- [x] OpenAI Embeddings API 장애 시 BM25 Fallback으로 후보·공식 링크 유지 자동 테스트 통과
- [x] 사용자 원금액·자격 원값·질문이 서비스 DB·검색 인덱스에 영구저장된 사례 0건
- [x] RE9 배포 QA·가상 페르소나 검증항목에 Hybrid·단계형 자격·최대 2회·fallback 흐름 반영
- [x] 후속 단계 영향 검토와 필요 수정 완료

---

## RE Stage 8.3 — 사용자 직접 서비스 검토·수정 반복

### 목표

사용자가 로컬 서비스 화면과 이용 흐름을 직접 확인하고, 수정이 필요한 위치와 기대 결과를 지정한다. Codex는 요청을 변경 원장에 구조화하고 항목별로 구현·검증한 뒤 사용자 재확인을 받는다. 이 단계에서는 외부 배포하지 않는다.

### 상태 기록

| 항목 | 기록 |
| --- | --- |
| 담당자 | 사용자 화면 검토·최종 판단 / Codex 기록·구현·회귀검증 |
| 시작일 | 2026-08-17 |
| 완료일 | 2026-08-21 |
| 상태 | 완료·종료 — V2로 전환했으며 재개하지 않음 |
| 선행조건 | Gate RE8.2 |
| 후속조건 | Gate 통과 완료; 후속 서비스 수정은 V2 원장에서만 진행 |
| 변경 원장 | `reports/re_stage8_3/service_review_log.md` |

### 운영 원칙

- [x] 외부 배포 중단 — 사용자가 별도로 다시 승인하기 전에는 실행하지 않음
- [x] 향후 배포 비용 기준을 지속 무료 요금제로 한정
- [x] 무료 플랫폼 선정·계정 연결·공개 URL 생성은 RE8.3 종료 뒤 별도 승인사항으로 분리
- [x] 스크린샷·이미지 분석 제외; 사용자가 직접 본 화면과 설명을 기준으로 수정
- [x] 사용자는 자연어로 화면·문제·원하는 결과를 전달할 수 있고 Codex가 변경 원장 형식으로 정리
- [x] 한 요청에 결과 의미·정책범위·개인정보·외부전송 등 중대한 선택이 포함되면 구현 전에 별도 승인
- [x] 변경하지 않은 Stage 0~6·RE1~RE8.2의 동결 Artifact는 보존

### 사용자 직접 검토 순서

- [ ] 로컬 서비스 실행과 첫 화면·안내 문구
- [ ] 사업장 위치·상권 지도·업종 선택
- [ ] 최근 매출·비용·현금·부채 입력
...
각 요청은 다음 필드로 기록한다. 사용자가 모든 필드를 직접 작성할 필요는 없다.
...
- [ ] 사용자 수정 요청 접수
- [ ] Codex가 요청 범위와 완료 기준을 변경 원장에 기록
- [ ] 다른 화면·계산·정책·개인정보 경계에 미치는 영향 확인
- [ ] 승인 범위 안의 코드·문서·테스트 수정
- [ ] 관련 집중 테스트와 필요한 범위의 전체 회귀검증
- [ ] 변경 파일·검증 결과·남은 한계를 사용자에게 보고
- [ ] 사용자가 같은 화면을 직접 재확인
- [ ] 미충족이면 동일 수정 ID를 유지해 반복하고, 충족이면 완료 처리

### Gate RE8.3

- [x] 사용자가 지정한 차단·중요 수정사항이 모두 완료됨
- [x] 열린 수정 ID와 알려진 핵심 회귀 오류가 0건임
- [x] 계산·자격·정책근거·순위 안전 계약이 유지됨
- [x] 관련 테스트와 전체 필수 회귀검증이 통과함
- [x] RE9 자료 재생성 요구는 V2 전환으로 폐기하고 기존 기준선만 보존함
- [x] 스크린샷·이미지 분석을 수행하지 않았음
- [x] 사용자가 서비스 직접 재확인 후 RE8.3 완전 종료·재개 금지를 명시적으로 승인함
- [x] 후속 단계 영향 검토 완료 — V2 계획·V2 원장으로 이관
...
| 시작일 | 2026-08-17 |
| 완료일 | 미정 |
| 상태 | 보류 — 수정 전 로컬 QA 기준선 보존·외부 배포 중단 |
| 선행조건 | Gate RE8.3 통과 완료; 현재는 V2 검토 종료와 사용자의 별도 배포 승인 필요 |

### P0 — 배포

- [x] 사용자의 배포 중단 요청 반영
- [x] 향후 플랫폼 비용 기준을 지속 무료 요금제로 한정
- [ ] V2 검토 종료 후 무료 플랫폼 후보의 메모리·절전·빌드·공개 URL·Secret 조건 비교
- [ ] 무료 조건을 벗어나거나 결제 전환 가능성이 있으면 실행 전 사용자 재승인
- [ ] 외부 공개 URL
- [ ] 제출 검증기간 접근
- [x] Health Check — 로컬 `/health` HTTP 200
- [ ] 재시작 후 샘플 정상
- [x] 환경변수 안전관리
- [x] 오류 로그와 개인정보 분리 계약
- [x] 의존성 고정 — `requirements-runtime.txt`
- [x] 배포 Runbook — `reports/re_stage9/deployment_runbook.md`

### P0 — 대표 시나리오

- [x] 상권 하락·낮은 부채
- [x] 안정 상권·높은 상환부담 — 8,000만원 중 공식 한도 5,000만원 부분대환
- [x] 상권 하락·현금 부족·기존 부채
- [x] 자격은 있지만 효과가 부족한 정책
- [x] 추가 확인조건이 있는 정책
- [x] 지급 지연
- [x] 외부모델 없는 상권·업종
- [x] 잘못된 입력
- [x] Hero 수치는 상세 일정이 있는 검증 샘플에서 재생성
- [x] 실제 사용자 사례가 아닌 가상 사업장임을 표시
...
- [ ] 외부 URL 접근
- [ ] 배포계정 OpenAI Usage tier·API 키·5초 Timeout·사용자 동작당 최대 2회·BM25 Fallback 확인
- [x] 승인 외부 전송 필드와 최대 30일 악용 모니터링 가능성 고지 확인
- [x] 샘플 결과 재현 — 가상 페르소나 `8/8`
- [x] 공식 링크 유효성 — 2026-08-17 공식 페이지 9개 URL·대표정책 10개 확인
- [x] 정책 접수기간·기준일 — 예산 소진·모집 완료는 현재 확인 필요 유지
- [x] 금융계산 재검산 — 부분대환·신규부채·현금요약 자동 대조
- [ ] 그래프·표·AI 설명 일치
- [x] 모델 한계 문구
- [x] 승인확률 오해 방지
- [x] 민감정보 비저장
- [ ] 모바일·데스크톱 확인
- [x] 제출문서·실제기능 일치 — 로컬 구현범위 기준
- [x] 10개 정책별 최소 2문항·총 20문항 이상 공식근거 검색 평가 — `20/20 Hit@5`
- [ ] 데모 직전 10개 정책 접수상태·변경공고·공식 링크 재확인
- [x] Rule ID·Chunk ID·검색점수 사용자 화면 노출 0건

### P0 — 발표자료 근거

- [x] 기존 서비스와의 차이
- [x] `지원조건 적용 시 돈이 언제까지 버티고 부채가 얼마나 늘어나는가` 핵심 문장
- [x] 대출 우선이 아닌 여섯 대응안의 동일 기준 비교
- [x] 대표 10개 정책을 금융구조별로 깊게 구현했다는 선정 이유
- [x] 무대응·개입 전후 데모
- [x] 신규대출의 현금·부채 동시효과
- [x] 최소부채 대응안
- [x] 공식 근거·가정 원장
- [x] Stage 0~6 기준선 보존 이유
- [x] 모델 교체 시 Gate 결과
- [x] 모델 미교체 시 한계·Fallback
- [x] 개인정보 보호
- [x] 확장 가능성
- [x] `AI가 미래를 예측하고 최적 정책을 추천` 표현 없음

### P0 — 가상 페르소나 시나리오 모의 검증

- [x] 실제 소상공인 모집 불가에 따라 모의 검증 대체 승인 — 2026-08-16
- [x] 고정 가상 페르소나 8개와 원천 입력 작성
- [x] 현금부족·고부채·사후환급·자격 미확인·부적격·접수종료·지원 부족·Embedding 최대 2회 후 BM25 Fallback·오류 입력 포함
- [ ] 기존 정책 목록 화면과 13주 생존 시뮬레이션 화면에 동일 입력 적용
- [x] Hybrid 후보 → 단계형 자격질문 → 검수 Event 동적 대안 → RE7 순위 Trace 보존
- [x] 규칙·계산 엔진으로 기대 최소부채 대안과 핵심 수치를 먼저 산출
- [x] 비교 전 정답 계약·입력 해시 동결
- [x] 현금 고갈 범위·시점, 신규부채, 월상환, 총이자, 확인조건 대조
- [x] 핵심 결과까지 필요한 화면 단계 수 기록 — 4단계
- [ ] 그래프·표·AI 설명의 숫자·의미 일치
- [x] 개인 매출예측·인과효과·승인확률·최적 정책 금지 표현 0건
- [x] 입력·정답 계약·실행 Trace·집계표 보존
- [x] 가상 만족도·이해도·자유 의견 생성 금지
- [x] LLM 가상 사용자 반응을 실제 사용자 근거로 사용 금지
- [x] 8개 상황 기대 대안·핵심 계산값 `8/8` 일치
- [x] 금지 표현 `0건`
- [x] 하나라도 실패하면 Gate 실패와 원인·수정사항 보존 — 검색 초안 `19/20` 실패 후 질문 구별어 보완 이력 기록
- [x] 실제 사용성 향상·정책 현실 효과로 확대 주장 금지
- [x] 불리한 모의 결과와 실패 사례도 보존·보고

### 완료 산출물

- [ ] 배포 URL
- [x] 기능명세서 PDF — `output/pdf/RE9_기능명세서.pdf`
- [x] 기획서 최종본 — `reports/re_stage9/기획서_제출본.md`
- [x] 샘플 입력
- [x] 예상 결과표
- [x] 배포 Runbook
- [x] 최종 QA 보고서
