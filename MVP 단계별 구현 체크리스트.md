...
| --- | --- |
| 상태 | 미착수 — 후보 모델 목록 미정 |
| 모델 선택 원칙 | 여러 후보를 동일한 4개 시간순 Fold에서 먼저 비교하고 최종 후보 1개를 선택함 |
| 튜닝 원칙 | 최종 후보로 선택된 모델에만 Optuna 튜닝을 수행함 |
| 금지 사항 | 후보 비교 전에 LightGBM 등 특정 모델을 최종 모델로 확정하지 않음 |
...
- [ ] 비교 결과로 최종 튜닝 후보 모델 1개와 선택 이유를 기록한다.
- [ ] 선택된 최종 후보 모델에만 Optuna 튜닝을 수행한다.
