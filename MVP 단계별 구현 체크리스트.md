...
- [ ] 원시 모델 점수를 절대 확률로 노출하지 않고 같은 업종 내 Percentile과 서울 전체 Percentile로 변환한다.
- [ ] 주 비교집단·보조 비교집단, 기준분기, 기준 점수분포와 `service_ranking_policy_version`을 설정 파일로 관리한다.
- [ ] `상위 N%`와 전체 순위를 반환하며 동점 처리 규칙을 고정한다.
- [ ] 이진 운영 임계값이 없으면 `위험/안전` 판정을 반환하지 않는다.
...
- [ ] “폐업확률 72%” 또는 “위험도 72%”처럼 절대확률로 오해되는 값을 표시하지 않는다.
...
- [ ] 결과에 기준 분기, 주·보조 비교집단, 두 Percentile의 의미, 정책 버전과 모델 적용 범위가 표시된다.
- [ ] 원시 모델 점수와 Percentile을 실제 폐업확률로 오해할 표현이 없다.
...
- [ ] 조건 미충족 정책을 먼저 제외한다.
- [ ] 남은 후보에 적합도 점수를 계산한다.
- [ ] 지역, 업종, 업력, 자금용도, 신청가능 여부와 상대 위험 프로필·금융부담의 정책목적 일치도를 반영한다.
- [ ] 상대 위험 Percentile은 추천 적합도 보조 신호로만 사용하며 정책 자격의 포함·제외 조건으로 사용하지 않는다.
